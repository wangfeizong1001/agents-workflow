...
export async function runCli(args: readonly string[], cwd: string): Promise<number> {
  ...
  program
    .command(initCommand.name)
    .description(initCommand.description)
    .action(async () => initCommand.handler(ctx));

  program
    .command(statusCommand.name)
    .description(statusCommand.description)
    .action(async () => statusCommand.handler(ctx));

  try {
    await program.parseAsync([...args]);
    return 0;
  } catch (err) {
    ctx.stderr(`错误: ${err instanceof Error ? err.message : String(err)}\n`);
    return 1;
  }
}
